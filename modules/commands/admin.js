...
`,
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != 100090987576608) return api.sendMessage(`» Xin lỗi! lệnh này chỉ admin mới dùng được`, event.threadID, event.messageID);
        ...
        case "addndh": { 
          if (event.senderID != 100090987576608) return api.sendMessage(`Cần quyền Admin chính để thực hiện lệnh`, event.threadID, event.messageID)
        ...
        case "removendh":{
          if (event.senderID != 100090987576608) return api.sendMessage(`Cần quyền Admin để thực hiện`, event.threadID, event.messageID)
